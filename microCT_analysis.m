...
mu_volume = (img_double / 65535) * (mu_max + mu_min);
...
% Save results in chosen directory
...
if strcmp(file_name, 'CT_21_1_01.nii')
    slices = [320, 240, 150, 80, 25];   % 15 mm, 35 mm, 55 mm, 55 mm (2 rods), 55 mm (3 rods)
elseif strcmp(file_name, 'CT_24_01_01.nii')
    slices = [407, 350, 250, 180, 120];
end

diameters = [15, 35, 55];                  
n_slices = 40;                             % number of slices used to reduce noise
...
for i = 1:3   % indexes of plexi-only sections (slice_indexes array)
    ...
    % Calculation of plexi center coordinates threshold to separate section from background
    t = (min(current_avg(:)) + max(current_avg(:))) / 2; 
    mask = current_avg > t;
    [cy, cx] = circleCenterMask(mask);      % centre of plexi section
                                            % y is the vertical direction
                                            % (cy is the x coordinate in the image)
...
half_width = 25;  % ROI will be (2*half_width + 1) x (2*half_width + 1)
...
for i = 1:3   %indexes of 15 mm, 35 mm, 55 mm plexi-only sections
    ...
    row_min = max( round(cy - half_width*i), 1 );
    row_max = min( round(cy + half_width*i), dim_y );
    col_min = max( round(cx - half_width*i), 1 );
    col_max = min( round(cx + half_width*i), dim_x );
...
%% Save all open figures to chosen directory


outDir = uigetdir(pwd, 'Select folder to save images');

if outDir == 0
    disp('Folder selection cancelled by user. No images were saved.');
    return
end
